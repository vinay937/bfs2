...
from .models import UserProfile, Subject, TheoryTeaches, LabTeaches, ElectiveTeaches, Rating, News 
...
def phone_otp(random_otp, phone, usn):
    '''
    Sends OTP to phone
    '''
    phone1 = phone
...
def password_update(random_otp, usn):
    """
...
def feedback_over_view(request):
...
def home_view(request):
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            if_admin(qs) #Checks if user is admin and redirects directly

            # Checks if done=False
            if not qs.profile.done :
                if qs:
                    # Checks if both email and phone doesn't exist
                    if not qs.email and not qs.profile.phone:
                        context = {
                        "error_msg": "Contact details incomplete. Contact coordinator"
                        }

                    # Checks if both email and phone exist
                    elif qs.email and qs.profile.phone:
                        phone_otp(random_otp, qs.profile.phone, usn)
                        email_otp(random_otp, qs)
                        password_update(random_otp, usn)
                        messages.error(request, "OTP " + random_otp + " sent to "+qs.profile.phone+" and "+qs.email)
                        return HttpResponseRedirect("/login/usn=" + usn)

                    # Checks if only phone exists    
                    elif qs.profile.phone and not qs.email:
                        phone_otp(random_otp, qs.profile.phone, usn)
                        password_update(random_otp, usn)
                        messages.error(request, "Email not found, OTP sent to "+qs.profile.phone)
                        return HttpResponseRedirect("/login/usn=" + usn)

                    # Checks if only email exists    
                    elif qs.email and not qs.profile.phone:
                        email_otp(random_otp, qs)
                        password_update(random_otp, usn)
